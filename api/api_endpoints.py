...
# # Endpoint: Check Snowflake SQL Query Syntax
# @app.post("/check_snowflake_sql_query_syntax")
...
#         tool = CheckQuerySyntaxTool(db=db)
#         result = tool.run(query.query)
#         logger.debug(f"Checked syntax for query: {query.query}.")
#         return {"result": result}
#     except Exception as e:
#         logger.error(f"Error checking syntax for query {query.query}: {e}")
#         raise HTTPException(status_code=500, detail=str(e))
